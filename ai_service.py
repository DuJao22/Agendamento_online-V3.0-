...
def _initialize_genai():
    ...
    if not GENAI_AVAILABLE:
        raise RuntimeError("Google Generative AI package not properly installed")
...
class ChatbotService:
    ...
    def processar_mensagem(self, mensagem, conversa):
        ...
        try:
            ...
            if estado == 'inicio' or estado == 'finalizado':
                ...
                return self._processar_solicitacao_anexo(mensagem, conversa, dados)
    ...
    def _processar_horarios(self, mensagem, conversa, dados):
        ...
        logging.info(f"DEBUG _processar_horarios: local_id={local_id}, especialidade_id={especialidade_id}, dados={dados}")
        ...
        logging.info(f"DEBUG query result: encontrou {len(rows) if rows else 0} registros")
        if rows:
            logging.info(f"DEBUG primeiro registro: {dict(rows[0])}")

        if not rows:
            logging.error(f"ERRO: Nenhum médico/horário encontrado para especialidade_id={especialidade_id}, local_id={local_id}")
            # Verificar se o problema é falta de médicos ou falta de horários
            medicos_especialidade = [m for m in Medico.find_active() if m.especialidade_id == especialidade_id]
            if not medicos_especialidade:
                return {
                    'success': False,
                    'message': "Não temos médicos desta especialidade disponíveis no momento. Por favor, escolha outra especialidade.",
                    'tipo': 'especialidades',
                    'proximo_estado': 'especialidade'
                }
            else:
                return {
                    'success': False,
                    'message': "Não há horários disponíveis para esta especialidade neste local. Tente outro local ou especialidade.",
                    'tipo': 'especialidades',
                    'proximo_estado': 'especialidade'
                }
        ...
        logging.info(f"DEBUG _processar_horarios: mensagem='{mensagem}', escolha={escolha}")
        ...
        if escolha:
            ...
            dados.update({
                ...
                'especialidade_nome': dados.get('especialidade_nome', 'Especialidade'),
            ...
            if especialidade and getattr(especialidade, 'requer_anexo', False):
                ...
                return {
                    'success': True,
                    'message': f"📋 **Agendamento Selecionado**\n\n" +
                    ...
                    'tipo': 'solicitacao_anexo',
                    'proximo_estado': 'solicitacao_anexo'
                }
            else:
    ...
    def _processar_solicitacao_anexo(self, mensagem, conversa, dados):
        ...
        if any(palavra in mensagem_lower for palavra in ['enviei', 'enviado', 'anexei', 'anexado', 'upload', 'foto', 'arquivo', 'ok', 'pronto', 'sim']):
            # Simular que o anexo foi recebido
            dados['anexo_recebido'] = True
            dados['anexo_nome'] = f"pedido_medico_{dados.get('paciente_id', 'user')}"
            conversa.set_dados(dados)
            conversa.estado = 'confirmacao'
        ...
        elif any(palavra in mensagem_lower for palavra in ['pular', 'sem anexo', 'não tenho', 'nao tenho', 'depois', 'não', 'nao']):
            ...
            return {
                'success': True,
                'message': f"⚠️ **Prosseguindo sem anexo**\n\n" +
                          f"📋 **Resumo do Agendamento**\n\n" +
                          f"👤 **Paciente:** {paciente.nome if paciente else 'N/A'}\n" +
                          f"🩺 **Médico:** {dados.get('medico_nome', 'N/A')}\n" +
                          f"🏥 **Especialidade:** {especialidade.nome if especialidade else 'N/A'}\n" +
        ...
        elif any(palavra in mensagem_lower for palavra in ['link', 'upload', 'anexo']):
            ...
            return {
                'success': True,
                'message': f"📎 **Link para Anexar Arquivo**\n\n" +
            ...
            return {
                'success': True,
                'message': f"📎 **Anexo de Pedido Médico Necessário**\n\n" +
        ...
        if any(palavra in mensagem_lower
            ...
            try:
                ...
                if dados.get('anexo_recebido'):
                    ...
                    especialidade = Especialidade.find_by_id(dados['especialidade_id'])
                    if especialidade and getattr(especialidade, 'requer_anexo', False):
                ...
                if local:
                    ...
                    endereco_completo = ", ".join(endereco_parts) if endereco_parts else local.nome
    ...
    def _gerar_horarios_disponiveis(self, rows):
        ...
        try:
            ...
            # DEBUG: Log inicial
            logging.info(f"DEBUG _gerar_horarios_disponiveis: hoje={hoje}, hora_atual={hora_atual}, rows={len(rows)}")
            
            # SIMPLIFICAÇÃO: Próximos 7 dias e horários mais variados
            horarios_fixos = ['08:00', '09:00', '10:00', '11:00', '13:00', '14:00', '15:00', '16:00', '17:00']
            ...
            for dia in range(50):  # Próximos 50 dias para dar mais opções
                ...
                logging.info(f"DEBUG processando dia {dia}: data={data_atual}, dia_semana={dia_semana}")
                
                # Pular fins de semana
                if dia_semana >= 5:
                    logging.info(f"DEBUG pulando fim de semana: dia_semana={dia_semana}")
                    continue
                
                # Processar médicos disponíveis para este dia
                for row in rows:
                    logging.info(f"DEBUG verificando médico: nome={row['nome']}, dia_semana_medico={row['dia_semana']}, dia_semana_atual={dia_semana}")
                    
                    if row['dia_semana'] != dia_semana:
                        continue
                        
                    medico_id = row['medico_id'] if 'medico_id' in row else row['id']
                    hora_inicio = datetime.strptime(row['hora_inicio'], '%H:%M').time()
                    hora_fim = datetime.strptime(row['hora_fim'], '%H:%M').time()
                    
                    logging.info(f"DEBUG médico {row['nome']} trabalha de {hora_inicio} às {hora_fim}")
                    
                    for hora_str in horarios_fixos:
                        if len(horarios) >= 8:  # Aumentar limite para dar mais opções
                            break
                            
                        slot_time = datetime.strptime(hora_str, '%H:%M').time()
                        
                        # Verificar se o horário está dentro do período de trabalho do médico
                        if slot_time < hora_inicio or slot_time >= hora_fim:
                            continue
                        
                        # Se é hoje, só horários futuros (com margem de 1 hora)
                        if data_atual == hoje:
                            hora_limite = datetime.now() + timedelta(hours=1)
                            slot_datetime = datetime.combine(data_atual, slot_time)
                            if slot_datetime <= hora_limite:
                                logging.info(f"DEBUG pulando horário passado: {slot_time} <= {hora_limite.time()}")
                                continue
                        
                        # Verificação simples de disponibilidade
                        if self._verificar_disponibilidade_slot_simples(medico_id, data_atual, slot_time):
                            # Buscar nome do local corretamente
                            nome_local = self._obter_nome_local(row['local_id'])
                            
                            # Buscar dados do médico para especialidade correta
                            dados_medico = self._obter_dados_medico(medico_id)
                            especialidade_nome = dados_medico['especialidade'] if dados_medico else 'Especialidade'
                            ...
                            horario_info = {
                                ...
                                'crm': row['crm'] if 'crm' in row.keys() else 'N/A',
                                'local_id': row['local_id'],
                                'local_nome': nome_local,
                                'data': data_atual.strftime('%Y-%m-%d'),
                                'hora': slot_time.strftime('%H:%M'),
                                'data_formatada': data_atual.strftime('%d/%m/%Y'),
                                'hora_formatada': slot_time.strftime('%H:%M'),
                                'dia_semana': ['Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado', 'Domingo'][dia_semana] if dia_semana < 7 else 'N/A',
                                'duracao': int(row['duracao_consulta']) if 'duracao_consulta' in row.keys() else 30,
                                'timestamp': datetime.combine(data_atual, slot_time).timestamp()
                            }
                            horarios.append(horario_info)
                            logging.info(f"DEBUG adicionado horário: {data_atual} {slot_time} - Dr. {row['nome']}")
                        else:
                            logging.info(f"DEBUG horário ocupado: {data_atual} {slot_time}")
            ...
            logging.info(f"DEBUG total de horários gerados: {len(horarios_ordenados)}")
            logging.info(f"DEBUG primeiro horário: {horarios_ordenados[0] if horarios_ordenados else 'nenhum'}")
            
            return horarios_ordenados[:8]
    ...
    def _interpretar_escolha_horario(self, mensagem, horarios_disponiveis):
        ...
        logging.info(f"DEBUG _interpretar_escolha_horario: mensagem='{mensagem_clean}'")
        logging.info(f"DEBUG horarios_disponiveis count: {len(horarios_disponiveis)}")

        # Primeiro, verificar se é uma escolha por número (1, 2, 3, etc.)
        import re
        numero_match = re.match(r'^(\d+)$', mensagem_clean)
        if numero_match:
            indice = int(numero_match.group(1)) - 1  # Converter para índice base 0
            logging.info(f"DEBUG numero_match: {numero_match.group(1)}, indice={indice}")
            if 0 <= indice < len(horarios_disponiveis):
                ...
                logging.warning(f"DEBUG índice {indice} fora do range [0-{len(horarios_disponiveis)-1}]")
                return None
...
• **Duração:** {horario['duracao']} minutos"""
        ...
        texto.append("• Ou apenas '**1**' para o primeiro horário, '**2**' para o segundo, etc.")
